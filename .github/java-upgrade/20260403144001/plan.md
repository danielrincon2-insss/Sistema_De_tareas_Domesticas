...
# Upgrade Plan: Sistema de Domésticas (20260403144001)

- **Generated**: April 3, 2026 <!-- replace with actual date and time when generating -->
- **HEAD Branch**: feature/asignar-tarea-semanal <!-- replace with actual head branch; use "N/A" when GIT_AVAILABLE=false -->
...
**JDKs**
- JDK 21: C:\Program Files\Java\jdk-21.0.10\bin (current project JDK, used by step 2)
...
| Spring Boot | 3.3.0 (root), 3.5.13 (hogar-service) | 3.5.x | Required for Java 25 compatibility |
| Maven Wrapper | 3.x | 4.0+ | Maven 3.x does not support Java 25 |
...
- Upgrade Spring Boot in root module from 3.3.0 to 3.5.x (required for Java 25 compatibility)
- Upgrade Maven Wrapper to 4.0+ (Maven 3.x does not support Java 25 compilation)
...
- Step 1: Setup Environment
  - **Rationale**: Install required JDK 25 and upgrade Maven wrapper to 4.0+ for compatibility with Java 25.
  - **Changes to Make**:
    - [ ] Install JDK 25
    - [ ] Upgrade Maven wrapper to 4.0+ in both root and hogar-service modules
  - **Verification**:
    - Command: `#appmod-list-jdks` to confirm JDK 25 installation
    - Expected: JDK 25 available

- Step 2: Setup Baseline
  - **Rationale**: Establish pre-upgrade compile and test results with current JDK 21.
  - **Changes to Make**:
    - [ ] Run baseline compilation
    - [ ] Run baseline tests
  - **Verification**:
    - Command: `mvnw clean test-compile && mvnw clean test`
    - JDK: C:\Program Files\Java\jdk-21.0.10\bin
    - Expected: Document SUCCESS/FAILURE, test pass rate

- Step 3: Upgrade Spring Boot in Root Module
  - **Rationale**: Upgrade Spring Boot to 3.5.x for Java 25 compatibility.
  - **Changes to Make**:
    - [ ] Update spring-boot-starter-parent version to 3.5.13 in root pom.xml
  - **Verification**:
...
- Step 4: Upgrade Java Version
  - **Rationale**: Change Java version to 25 in pom.xml files.
  - **Changes to Make**:
    - [ ] Update java.version to 25 in root pom.xml
    - [ ] Update java.version to 25 in hogar-service pom.xml
  - **Verification**:
    - Command: `mvnw clean test-compile`
    - JDK: <JDK 25 path>
    - Expected: Compilation SUCCESS
...
- Step 5: Final Validation
  - **Rationale**: Verify all upgrade goals met, project compiles and tests pass with Java 25.
  - **Changes to Make**:
    - [ ] Verify java.version is 25 in pom.xml files
...
- **Multi-module Project with Different Spring Boot Versions**
  - **Challenge**: Root module has Spring Boot 3.3.0, hogar-service has 3.5.13, need to align for Java 25 compatibility.
  - **Strategy**: Upgrade root to 3.5.x to match hogar-service.

- **Java 25 Compatibility**
  - **Challenge**: Java 25 is new, potential compatibility issues with dependencies.
  - **Strategy**: Use Spring Boot 3.5.x which supports Java 25.
